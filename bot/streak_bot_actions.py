...
# ══════════════════════════════════════════════════════════
#  GitHub Actions version of streak_bot
#  — No git commands here, Actions handles commit & push
#  — API key comes from GitHub Secrets automatically
# ══════════════════════════════════════════════════════════

USE_AI  = True   # Always True in Actions — key comes from GitHub Secrets
API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")


# ── Built-in LeetCode problems (fallback if AI fails) ────
LEETCODE_TEMPLATES = [
    {
...
'''
    },
    {
        ...
        "description": "Given a string with brackets, determine if it is valid.",
...
'''
    },
    {
        ...
        "description": "You can climb 1 or 2 steps. How many ways to reach top of n stairs?",
...
def generate_with_ai(content_type: str) -> dict:
    ...
    try:
        ...
        if not API_KEY:
            print("[AI] No API key found — using templates.")
            return None
...
{{"title":"...","difficulty":"Easy|Medium|Hard","description":"1-2 sentence description","solution":"complete working Python solution with test cases"}}"""
        else:
            prompt = f"""Generate a useful Python utility snippet for {today}.
Return ONLY valid JSON (no markdown, no backticks):
{{"title":"...","code":"complete working Python code with comments and examples"}}"""
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            print(f"[AI] Generated: {result.get('title', '?')}")
            return result
...
def write_file(rel_path: str, content: str):
    """Write file relative to repo root (where Actions checks out)."""
    full = Path(rel_path)
...
def run():
    ...
    if content_type == "leetcode":
        ...
        print(f"[DONE] LeetCode — {problem['title']} [{problem.get('difficulty','?')}]")
    else:
        ...
        write_file(fname, fcontent)
        print(f"[DONE] Snippet — {snippet['title']}")

    print(f"\n✅  File ready for commit → {fname}")
